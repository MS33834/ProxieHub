...
[![Last Commit](https://img.shields.io/github/last-commit/MS33834/ProxieHub)](CHANGELOG.md)
...
| 多格式订阅 | 同时输出 Clash、V2Ray、HTTP(S)/SOCKS4/SOCKS5 三种常用订阅格式。 |
...
- **首页**：项目概览、实时统计、FAQ。
- **订阅**：一键复制 Clash / V2Ray / HTTP(S)/SOCKS4/SOCKS5 订阅链接。
...
主要来源为 GitHub Raw 上的公开订阅文件，部分为纯文本，部分为 Base64 编码。当前启用约 28 个节点源，覆盖以下协议：
...
已启用多个公开代理列表，包括 `proxifly/free-proxy-list`、`TheSpeedX/PROXY-List`、`ErcinDedeoglu/proxies`、`jetkai/proxy-list` 等，协议覆盖 `http`、`https`、`socks4`、`socks5`。另有 gfpcom 系列大型代理列表默认关闭，可按需启用。
...
```text
┌─────────────────┐     ┌─────────────────┐     ┌─────────────────┐
│   数据源配置     │ --> │   爬虫/解析器    │ --> │  验证/格式化器   │
│ sources.json    │     │ crawler/parser  │     │ verifier/       │
└─────────────────┘     └─────────────────┘     │ formatter       │
                                                └────────┬────────┘
                                                         │
                                ┌────────────────────────┼────────────────────────┐
                                ▼                        ▼                        ▼
                          ┌──────────┐            ┌──────────┐            ┌──────────┐
                          │clash.yaml│            │v2ray.txt │            │proxies.txt│
                          └──────────┘            └──────────┘            └──────────┘
```

> 说明：`config/sources.json` 定义所有公开数据源；`crawler.py` / `parser.py` 负责抓取与解析；`verifier.py` / `formatter.py` 负责校验、去重与多格式输出，最终生成 `nodes/` 下的 Clash、V2Ray 与 HTTP(S)/SOCKS4/SOCKS5 订阅文件。
...
| `PROXIEHUB_MAX_PROXIES` | `200` | 输出中保留的最大 HTTP(S)/SOCKS4/SOCKS5 代理数 |
...
无论你是想修复 Bug、新增数据源、改进脚本，还是完善文档，都可以按以下流程快速参与：

1. **Fork 仓库**：点击 GitHub 页面右上角 **Fork**，将项目复制到你的账号下。
2. **修改配置或代码**：
   - 新增/修改数据源：编辑 `config/sources.json`。
   - 改进代码：在 `scripts/` 或 `tests/` 中修改，保持函数单一职责并补充测试。
   - 改进文档：在 `docs-site/` 或 `docs/` 中修改。
3. **本地验证**：运行 `make test`，确保所有单元测试通过；如涉及代码变更，建议同时运行 `make lint`。
4. **提交 Pull Request**：推送到你的 Fork 后，向原仓库提交 PR，清晰描述改动原因与测试结果。
5. **等待合并**：维护者会在 CI 检查通过后进行 Review，确认无误后合并到 `main` 分支。

> 首次贡献建议先阅读 [CONTRIBUTING.md](CONTRIBUTING.md)、[docs-site/contributing.md](docs-site/contributing.md) 与 [CODE_OF_CONDUCT.md](CODE_OF_CONDUCT.md)。

---

## 免责声明

**重要提示：在使用本项目之前，请仔细阅读以下条款。**

- **资源聚合性质**：本项目仅对互联网上公开可访问的代理与节点资源进行自动化聚合、解析与格式转换，**项目本身不运营、不提供任何代理或 VPN 节点**，也不对第三方数据源的内容进行实质审查。
- **不保证服务质量**：维护者不对任何节点的可用性、稳定性、安全性、隐私性、 anonymity 或法律合规性作出任何明示或暗示的保证。公开节点可能随时失效、被封锁或被篡改。
- **用户自行承担风险**：使用本项目生成的订阅文件、节点列表或任何输出内容，即表示你理解并同意自行承担由此产生的全部风险。建议不要在免费节点环境中登录银行、支付、社交媒体或其他敏感账户。
- **禁止违法用途**：本项目仅供网络协议学习、安全测试与隐私技术研究使用。严禁将本项目或其输出用于任何违反所在国家或地区法律法规、侵犯他人权益、实施网络攻击、垃圾邮件、欺诈或其他非法活动的行为。
- **责任限制**：在任何情况下，项目维护者、贡献者或版权持有者均不对因使用或无法使用本项目而产生的任何直接、间接、附带、特殊或后果性损害承担责任。

完整安全策略与漏洞报告方式请见 [SECURITY.md](SECURITY.md)。

---

## 相关链接

- **前端站点**：<https://ms33834.github.io/ProxieHub/>
- **文档站点**：<https://ms33834.github.io/ProxieHub/docs/>
- **GitHub Issues**：<https://github.com/MS33834/ProxieHub/issues>
- **GitCode 镜像**：<https://gitcode.com/badhope/ProxieHub>
...
- 想新增数据源？请先阅读 [docs/data-source-guide.md](docs/data-source-guide.md) 或网站上的 [数据源贡献指南](https://ms33834.github.io/ProxieHub/sources/guide)。
...
The project itself does **not** operate any proxy or VPN servers. It fetches publicly available sources, parses proxy links, optionally verifies connectivity, and generates multi-format subscription files (Clash, V2Ray, HTTP(S)/SOCKS4/SOCKS5). A Next.js static site and a VitePress documentation site are also included.
