...
本页面说明如何查看 ProxieHub 的当前状态，解释 `nodes/` 目录下各输出文件含义，并说明 GitHub Actions 的运行频率与同步策略。
...
| Deploy Web | `deploy-web.yml` | 构建并部署主站与文档站到 GitHub Pages |
...
部署后的 Next.js 主站会从 `nodes/clash.yaml` 与 `config/sources.json` 读取统计信息，展示：
...
访问路径：`https://<owner>.github.io/ProxieHub/`

## `nodes/` 目录输出文件说明

`nodes/` 目录是 ProxieHub 的核心产物目录，所有订阅链接都从这里获取。
...
- **执行内容**：
  ...
  - Web 任务：Next.js 依赖安装、ESLint 检查、静态导出构建。

### 站点部署

- **工作流**：`.github/workflows/deploy-web.yml`
- **触发条件**：
  - `web/`、`docs-site/`、`nodes/`、`config/sources.json` 任一文件变更
  - `Update Nodes` 工作流成功完成后
  - 手动触发
- **执行内容**：
  1. 构建 Next.js 主站到 `web/dist`。
  2. 构建 VitePress 文档站到 `docs-site/.vitepress/dist`。
  3. 将文档站产物复制到 `web/dist/docs/`。
