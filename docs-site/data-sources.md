...
ProxieHub 不生产、不运营任何代理节点，所有内容均来自互联网公开渠道。数据源配置完全公开，任何人都可以审计、替换或贡献新的公开源。

## 配置文件

所有数据源都登记在仓库根目录的 [`config/sources.json`](https://github.com/MS33834/ProxieHub/blob/main/config/sources.json) 中。该文件分为两大类：

- `free_node_sources`：V2Ray/Clash 系免费节点订阅源。
- `free_proxy_apis`：HTTP/SOCKS4/SOCKS5 公开代理列表 API。
...
| `ebrasha-free-v2ray-public-list` | GitHub Raw | 30min | vless, vmess | 每 30 分钟更新，明文 vless/vmess 链接 |
...
此外，配置文件中还包含若干默认禁用的源（如更新停滞或文件体积过大的源）。你可以在本地启用它们，但需注意稳定性与抓取耗时。

## 代理源（free_proxy_apis）

| 名称 | 类型 | 更新频率 | 协议 | 说明 |
|---|---|---|---|---|
| `proxifly-free-proxy-list` | GitHub Raw | 5min | http, https, socks4, socks5 | 公开 HTTP/SOCKS 代理列表，每 5 分钟验证一次 |
| `gfpcom-http` | GitHub Raw | hourly | http | 每小时更新的公开 HTTP 代理列表（文件较大） |
| `gfpcom-https` | GitHub Raw | hourly | https | 每小时更新的公开 HTTPS 代理列表（文件较大） |
| `gfpcom-socks4` | GitHub Raw | hourly | socks4 | 每小时更新的公开 SOCKS4 代理列表（文件较大） |
| `gfpcom-socks5` | GitHub Raw | hourly | socks5 | 每小时更新的公开 SOCKS5 代理列表（文件较大） |
